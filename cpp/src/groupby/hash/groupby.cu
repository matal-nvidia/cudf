...
int find_grid_size(FuncType func, int block_size, cudf::size_type num_input_rows, int num_sms)
{
  ...
  int needed_active_blocks = util::div_rounding_up_safe(num_input_rows, block_size);
...
{
  auto active_blocks_per_sm = util::div_rounding_up_safe(grid_size, num_sms);
                               ...
                               bool skip_rows_with_nulls)
{
  ...
  auto shmem_agg_pointer_size =
    util::round_up_safe(sizeof(std::byte*) * output_values.num_columns(), 8UL);
...
rmm::device_uvector<size_type> compute_single_pass_aggs(
                               ...
                               rmm::device_async_resource_ref mr)
{
  ...
  auto const comparator_helper = [&](auto const d_key_equal) {
    ...
    auto gather_map = compute_single_pass_aggs(keys,
                                               requests,
                                               &sparse_results,
                                               set,
                                               keys_have_nulls,
